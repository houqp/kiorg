...
"#;
    ...
    let shortcuts = loaded_config.shortcuts.as_ref().expect("Should have shortcuts");
    
    // User override: MoveDown should be 's'
    let move_down_shortcuts = shortcuts.get(&kiorg::config::shortcuts::ShortcutAction::MoveDown)
    ...
    let move_up_shortcuts = shortcuts.get(&kiorg::config::shortcuts::ShortcutAction::MoveUp)
    ...
    assert!(delete_shortcuts.is_none(), "Should not have default shortcuts in loaded config");
    ...
    let saved_content = fs::read_to_string(config_dir.join("config.toml"))
        .expect("Should read saved config file");
    ...
    assert!(saved_content.contains("key = \"s\""), "Should save user override for MoveDown");
    assert!(saved_content.contains("key = \"w\""), "Should save user override for MoveUp");
    ...
    assert!(!saved_content.contains("DeleteEntry"), "Should not save default shortcuts that weren't overridden");
    assert!(!saved_content.contains("key = \"d\""), "Should not save default 'd' shortcut for DeleteEntry");
    
    // It should not contain all the default shortcuts
    assert!(!saved_content.contains("AddEntry"), "Should not save default AddEntry shortcut");
    assert!(!saved_content.contains("CopyEntry"), "Should not save default CopyEntry shortcut");
    
    // The saved config should be much smaller than a config with all defaults
    let line_count = saved_content.lines().count();
    assert!(line_count < 20, "Saved config should be concise, got {} lines", line_count);
}
...
"#;
    ...
    assert!(!saved_content.contains("[shortcuts."), "Should not save shortcuts section when no user overrides");
    assert!(!saved_content.contains("MoveDown"), "Should not save default MoveDown shortcut");
    assert!(!saved_content.contains("key = \"j\""), "Should not save default 'j' shortcut");
    
    // It should contain the theme setting
    assert!(saved_content.contains("theme"), "Should preserve other config settings");
}
...
"#;
    ...
    assert!(saved_content.contains("key = \"m\""), "Should save user's shortcut modification");
    assert!(saved_content.contains("MoveDown"), "Should save the action that was modified");
    
    // It should not contain other default shortcuts
    assert!(!saved_content.contains("DeleteEntry"), "Should not save unmodified default shortcuts");
    assert!(!saved_content.contains("AddEntry"), "Should not save unmodified default shortcuts");
}
